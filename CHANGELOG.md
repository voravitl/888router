...
- **`open-sse/translator/concerns/thinkingUnified.js`:** `toLevel()` clamps `ultra → max` so no wire format ever sees the invalid string. Claude-adaptive `output_config.effort` now maps every level onto the native enum (low/medium/high only, `minimal→low`, beyond-`high`→`high`, `auto` omits `output_config`). Budget formats (`claude-budget`, `gemini-budget`, `qwen`) now clamp the budget to `caps.maxOutput` (strict `<`, so `budget_tokens` never equals `max_tokens` — Anthropic rejects that); `hunyuan` (maxOutput 262144) passes 160000 through untouched. Result per provider: OpenAI `reasoning_effort=max`, Claude adaptive `effort=high`, Claude/gemini/qwen budgets clamp to the model's output cap, Gemini `thinkingLevel=high`, Kimi/DeepSeek `max`, Step `high` — instead of upstream 400s (openai/gemini/claude/step) or silent collapse (kimi).
