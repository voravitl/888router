...
### Fix: model-sync surfaced a bare status, hiding the upstream reason (#279)

- **`src/lib/upstreamErrorDetail.js` (new):** `formatModelsFetchError(status, body)` appends the upstream's own reason to the client-facing error, so `Failed to fetch models: 403` becomes `Failed to fetch models: 403 — You have run out of credits or need a Grok subscription.` Reads the reason out of the common payload shapes (`message`, `error_description`, `detail`, `reason`, `error`, `code`, including the nested `{ error: { message } }` form), falls back to raw text for non-JSON bodies (HTML error pages, truncated streams), collapses whitespace to one line, and caps the result at 300 chars. Returns the bare status unchanged when the body carries nothing usable, so the message never regresses below what it was.
- **Why it mattered:** the distinguishing text was already read into `errorText` and logged server-side, then dropped before the response. From the dashboard a billing 403 (`out of credits`) and an auth 403 (`token rejected`) were the same string, which is what made #272 hard to diagnose — no token refresh can clear a billing 403, but nothing in the UI said so.
- **`src/app/api/providers/[id]/models/route.js`:** all four upstream-failure paths (OpenAI-compatible, Anthropic-compatible, `ollama-local`, and the generic OAuth refresh-retry path) now use the helper. Each already had `errorText` in scope — verified per call site, not assumed.
- **Security boundary is a field allowlist, not a regex denylist.** Only values read from a known set of human-message fields are ever emitted, and a payload that parses but carries no such field yields **nothing** rather than falling back to the raw body — that fallback would have leaked internal hostnames, request echoes, and credentials sitting in unrecognised fields. Regex redaction (auth schemes `Basic`/`Bearer`/`Digest`/`Negotiate`/`Token`, quoted credential-named JSON fields, vendor formats `sk-`/`xai-`/`glpat-`/`ghp_`/`github_pat_`/`AIza`/`xox*`/`AKIA`, JWTs, bare ≥32-char hex) runs on top as defence in depth, never as the boundary.
- **Raw upstream bodies are no longer written to the server log either.** All five log sites now pass through `safeLogDetail()`, which logs the sanitized detail or `<no recognisable message; N bytes withheld>`. Logging the raw body verbatim persisted any echoed credential to disk — inconsistent with the module's own premise.
- **Also fixed a direct leak found while wiring this up:** the Gemini CLI fallback path interpolated the raw upstream body straight into a `warning` field returned **to the client** (`route.js`). It now carries the same sanitized detail as the error paths.
- **Four over-redaction and leak bugs were caught by review and tests before merge**, all from treating a short regex denylist as a boundary: `Authorization: Bearer <token>` leaked the token (`\S+` was satisfied by the word `Bearer`); `Authorization: Basic <b64>` leaked the credential entirely; `{"access_token": "..."}` and `{"client_secret": "..."}` were not matched at all; and matching a keyword with an *optional* separator turned `access token could not be validated` into `access token [redacted] not be validated` while vendor prefixes without their delimiter redacted the ordinary words `skyscraper` and `ghostwriter`. Values that are plainly a state (`token: expired`, `API key: missing`) are now preserved — redacting those destroys the diagnostic this change exists to deliver.
- **Tests:** `tests/unit/upstream-error-detail.test.js` (24 new) covers the xAI billing case, auth-vs-billing distinguishability, nested payloads, non-JSON and HTML bodies, the unrecognised-payload case, a credential parked in a non-message field, the length cap, malformed input, every redaction class, `safeLogDetail`, and a regression set pinning each over-redaction above. Three assertions in `tests/unit/models-route-oauth-refresh.test.js` were updated for the intentionally changed string. Full suite: 1792 pass / 16 fail, all 16 pre-existing on a clean checkout (13 `AUDIT-*`, 3 `GOLDEN buildHeaders` — verified by stash-and-rerun).
