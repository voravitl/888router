...
- **Secrets are redacted before the body is returned.** A 401/403 body is exactly where an upstream is most likely to echo back the credential it just rejected, so the helper strips bearer/authorization values, vendor key formats (`sk-`, `xai-`, `ghp_`, `AIza`), JWTs, and bare ≥32-char hex runs.
- **Two redaction bugs were caught by the tests before merge, both from over-broad matching:** `Authorization: Bearer <token>` initially leaked the token (the `\S+` was satisfied by the word `Bearer`), and requiring only an *optional* `:`/`=` separator turned `access token could not be validated` into `access token [redacted] not be validated` — mangling the exact class of message this change exists to surface. The keyword-with-separator and bare-keyword cases are now matched separately, the latter behind a ≥16-char credential-shaped floor, with regression tests pinning both.
- **Tests:** `tests/unit/upstream-error-detail.test.js` (15 new) covers the xAI billing case, auth-vs-billing distinguishability, nested payloads, non-JSON bodies, the length cap, malformed input, and each redaction class. Three assertions in `tests/unit/models-route-oauth-refresh.test.js` were updated for the intentionally changed string. Full suite: 1785 pass / 16 fail, all 16 pre-existing on a clean checkout (13 `AUDIT-*`, 3 `GOLDEN buildHeaders` — verified by stash-and-rerun).
