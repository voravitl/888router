...
function wrapInCloudCodeEnvelope(model, geminiCLI, credentials = null, isAntigravity = false) {
  ...
  const envelope = {
    ...
    request: {
      sessionId: toNumericSessionId(credentials?._currentSessionId || credentials?._clientSessionId) || (isAntigravity ? deriveSessionId(credentials?.email || credentials?.connectionId) : generateSessionId()),
...
function wrapInCloudCodeEnvelopeForClaude(model, claudeRequest, credentials = null, signature = DEFAULT_THINKING_AG_SIGNATURE) {
  ...
  const envelope = {
    ...
    requestType: "agent",
    request: {
      sessionId: toNumericSessionId(credentials?._currentSessionId || credentials?._clientSessionId) || deriveSessionId(credentials?.email || credentials?.connectionId),
      contents: [],
      generationConfig: {
