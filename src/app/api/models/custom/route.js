...
export async function POST(request) {
  try {
    const { providerAlias, id, type, name, source, contextLength } = await request.json();
    if (!providerAlias || !id) {
      return NextResponse.json({ error: "providerAlias and id required" }, { status: 400 });
    }
    const added = await addCustomModel({ providerAlias, id, type: type || "llm", name, source: source || "custom", contextLength });
