...
describe("assertPublicUrl: literal hostname/IP bypasses from #3714", () => {
  ...
  it("blocks non-http(s) protocols", () => {
    for (const url of ["file:///etc/passwd", "ftp://example.com/test", "gopher://example.com/"]) {
      expect(() => assertPublicUrl(url), url).toThrow();
    }
  });

  it("blocks 6to4 addresses embedding private IPv4", () => {
    expect(() => assertPublicUrl("http://[2002:7f00:1::1]/")).toThrow();
    expect(() => assertPublicUrl("http://[2002:0a00:1::1]/")).toThrow();
  });
...
describe("assertPublicUrlResolved: DNS-resolving hostname bypass from #3714", () => {
  ...
  it("blocks non-http(s) protocols on the resolved path", async () => {
    for (const url of ["file:///etc/passwd", "ftp://example.com/x", "gopher://8.8.8.8/"]) {
      await expect(assertPublicUrlResolved(url)).rejects.toThrow(/non-http\(s\) protocol/);
    }
