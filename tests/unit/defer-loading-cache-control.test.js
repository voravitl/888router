...
describe("defer_loading tools never carry cache_control (#3567)", () => {
  ...
  it("prepareClaudeRequest: dropped non-function tools do not shift anchor onto deferred tail", () => {
    const out = prepareClaudeRequest({
      model: "claude-sonnet-4.5",
      messages: [{ role: "user", content: "hi" }],
      tools: [
        ...
        tool("a"),
        tool("mcp__x__y", { defer_loading: true }),
      ],
    }, "claude");
  ...
  it("injectPromptCaching: skips deferred tail tool and anchors on previous cacheable tool", async () => {
    const { injectPromptCaching } = await import("../../open-sse/translator/concerns/promptCache.js");
    const body = {
      messages: [{ role: "user", content: "hi" }],
      tools: [tool("a"), tool("b"), tool("mcp__x__y", { defer_loading: true })],
    };

    injectPromptCaching(body, "claude");

    expect(body.tools[2].cache_control).toBeUndefined();
    expect(body.tools[1].cache_control).toEqual({ type: "ephemeral", ttl: "1h" });
  });

  it("injectPromptCaching: strips cache_control if client set it on deferred tool", async () => {
    const { injectPromptCaching } = await import("../../open-sse/translator/concerns/promptCache.js");
    const body = {
      messages: [{ role: "user", content: "hi" }],
      tools: [tool("mcp__x__y", { defer_loading: true, cache_control: { type: "ephemeral" } })],
    };
  ...
  it("pipeline: prepareClaudeRequest -> injectPromptCaching -> anchorClaudeCache keeps anchor off deferred tools", async () => {
    const { injectPromptCaching } = await import("../../open-sse/translator/concerns/promptCache.js");
    const initial = {
      model: "claude-sonnet-4.5",
      messages: [{ role: "user", content: "hi" }],
      tools: [tool("read_file"), tool("mcp__defer", { defer_loading: true })],
    };

    const prepared = prepareClaudeRequest(initial, "claude");
    injectPromptCaching(prepared, "claude");
    anchorClaudeCache(prepared);

    expect(prepared.tools[1].defer_loading).toBe(true);
    expect(prepared.tools[1].cache_control).toBeUndefined();
    expect(prepared.tools[0].cache_control).toEqual({ type: "ephemeral", ttl: "1h" });
  });
});
