...
describe.skipIf(!handleEmbeddings)("handleEmbeddings — CORS OPTIONS", () => {
...
describe.skipIf(!handleEmbeddings)("handleEmbeddings — authentication", () => {
  beforeEach(() => {
...
describe.skipIf(!handleEmbeddings)("handleEmbeddings — body validation", () => {
  beforeEach(() => {
    vi.mocked(extractBearerToken).mockReturnValue(VALID_API_KEY);
    vi.mocked(parseApiKey).mockResolvedValue({ machineId: MACHINE_ID, keyId: "key01", isNewFormat: true });
    vi.mocked(getMachineData).mockResolvedValue(makeMachineData());
...
describe.skipIf(!handleEmbeddings)("handleEmbeddings — valid request (happy path)", () => {
  beforeEach(() => {
    vi.mocked(extractBearerToken).mockReturnValue(VALID_API_KEY);
    vi.mocked(parseApiKey).mockResolvedValue({ machineId: MACHINE_ID, keyId: "key01", isNewFormat: true });
    vi.mocked(getMachineData).mockResolvedValue(makeMachineData());
...
describe.skipIf(!handleEmbeddings)("handleEmbeddings — rate limit fallback", () => {
  beforeEach(() => {
    vi.mocked(extractBearerToken).mockReturnValue(VALID_API_KEY);
    vi.mocked(parseApiKey).mockResolvedValue({ machineId: MACHINE_ID, keyId: "key01", isNewFormat: true });
    vi.mocked(getModelInfoCore).mockResolvedValue({ provider: "openai", model: "text-embedding-ada-002" });
...
describe.skipIf(!handleEmbeddings)("handleEmbeddings — machineId override path", () => {
  beforeEach(() => {
